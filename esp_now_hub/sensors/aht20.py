...
class AHT20:
    ...
    def __init__(self, scl, sda, initialize=True):
        self._i2c = machine.SoftI2C(scl=machine.Pin(scl), sda=machine.Pin(sda))
        if initialize:
            time.sleep(self.SETUP_TIME)
            if not self._get_status() & 0x08:
                self._i2c.writeto(self.ADDRESS, self.INIT_CMD)
                time.sleep(self.INIT_TIME)

    def _get_status(self):
        self._i2c.writeto(self.ADDRESS, self.STATUS_CMD)
        return self._i2c.readfrom(self.ADDRESS, 1)[0]

    def get_measure(self):
        """Return humidity (0-100), temperature (Celsius)."""
        self._i2c.writeto(self.ADDRESS, self.MEASURE_CMD)
        time.sleep(self.MEASURE_TIME)
        while self._get_status() & 0x80:
            time.sleep(0.01)
        data = self._i2c.readfrom(self.ADDRESS, 6)
